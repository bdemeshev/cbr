...
Doc2Df <- function(doc, headtag) {
  cleanup <- XML::getNodeSet(doc, paste0("//", headtag))  # создаем список, разбивая по headtag
...
RepoDebtUSDXML <- function(fromDate, ToDate) {
  ...
  df <- xts::xts(apply(select(df, -D0), 2, as.numeric), order.by = df$D0)
  return(df)
...
Isoterm <- function(onDate, ToDate, I_Day) {
  ...
  df <- xts::xts(df$val, order.by = df$day)
  return(df)
}
...
AuctionsXML <- function(DateFrom, DateTo) {
  ...
  response <- h$value()  #получение ответа от сервера
  doc <- XML::xmlInternalTreeParse(response)  # создание XML-дерева
  cleanup <- XML::getNodeSet(doc, "//AU")  #выделение ячеек с данными
...
CouponsXML <- function(DateFrom, DateTo) {
  ...
  response <- h$value()  #получение ответа от сервера
  doc <- XML::xmlInternalTreeParse(response)  # создание XML-дерева
  cleanup <- XML::getNodeSet(doc, "//CP")  #выделение ячеек с данными
...
GKOOFZ_AnalitXML <- function(DateFrom, DateTo) {
  ...
  response <- h$value()  #получение ответа от сервера
  doc <- XML::xmlInternalTreeParse(response)  # создание XML-дерева
  cleanup <- XML::getNodeSet(doc, "//GA")  #выделение ячеек с данными
  vars <- names(cleanup[[1]])  # получить заголовки переменыными
  # сформировать датафрейм исходя из количества переменных и их их длины
  df <- as.data.frame(matrix(nrow = length(cleanup), ncol = length(vars),
    dimnames = list(NULL, vars)))
...
GetCursDynamicXML <- function(FromDate, ToDate, ValutaCode) {
  ...
  cleanup <- XML::getNodeSet(doc, "//ValuteCursDynamic")

  df[, "CursDate"] <- as.Date(as.POSIXct(df[, "CursDate"]))
  df <- df[, c(1, 4)]
  df <- xts::xts(as.numeric(as.character(df[, 2])), order.by = df[, 1])
  return(df)
}
...
# function(OnDate, ToDate, ValutaCode, I_Day){ h <- RCurl::basicTextGatherer()
...
# 'Content-Type' = 'text/xml; charset=utf-8') RCurl::curlPerform(url = url,
# httpheader = HeaderFields, postfields = body, writefunction =
# h$update) response <- h$value() doc <- XML::xmlInternalTreeParse(response)
# cleanup <- XML::getNodeSet(doc, '//It') vars <- names(cleanup[[1]]) df <-
...
GKOOFZ_AnalitXML <- function(DateFrom, DateTo) {
  ...
  doc <- XML::xmlInternalTreeParse(response)  # создание XML-дерева
  cleanup <- XML::getNodeSet(doc, "//GA")  #выделение ячеек с данными
  vars <- names(cleanup[[1]])  # получить заголовки переменных
  # сформировать датафрейм исходя из количества переменных и их их длины
  df <- as.data.frame(matrix(nrow = length(cleanup), ncol = length(vars),
                             dimnames = list(NULL, vars)))
  # пройтись по всем переменным в цикле
