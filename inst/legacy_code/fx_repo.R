...
chart.Events(fx_repo/1000, dates = end.date, ylab = "bln $", main = "Общая задолженность банков по операциям валютного репо")

dygraph(fx_repo/1000, main = "Задолженность банков по валютному репо",
  ylab = "млрд $")
write.csv(xtsToDf(fx_repo), file = "data_out/fx_repo.csv")


reserves <- mrrf7DXML(start.date, end.date)
reserves <- merge.xts(reserves, fx_repo/1000, join = "left")
